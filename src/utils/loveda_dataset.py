...
DATA_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "..", "..", "data/loveda"
)
...
class LoveDA(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(self.cls_filepath_list) > 0:
            ...
            return image, dict(
                cls=mask, fname=os.path.basename(self.rgb_filepath_list[idx])
            )
        else:
            if self.transforms is not None:
...
class LoveDALoader(DataLoader, ConfigurableMixin):
    def __init__(self, config):
        ...
        dataset = LoveDA(
            self.config.image_dir, self.config.mask_dir, self.config.transforms
        )
        ...
        super(LoveDALoader, self).__init__(
            dataset,
            ...
            dict(
                ...
                transforms=Compose(
                    [
                        ...
                        Normalize(
                            mean=(), std=(), max_pixel_value=1, always_apply=True
                        ),
                        ToTensorV2(),
                    ]
                ),
